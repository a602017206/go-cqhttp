...
build: checks ## builds minio to $(PWD)
	@echo "Building minio binary to './go-cqhttp'"
	@CGO_ENABLED=0 go build -tags kqueue -trimpath --ldflags "$(LDFLAGS)" -o $(PWD)/go-cqhttp 1>/dev/null

build-linux: checks ## builds minio to $(PWD)
	@echo "Building minio binary to './go-cqhttp'"
	@CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -tags kqueue -trimpath --ldflags "$(LDFLAGS)" -o $(PWD)/go-cqhttp 1>/dev/null

build-windows: checks ## builds minio to $(PWD)
	@echo "Building minio binary to './go-cqhttp'"
	@CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -tags kqueue -trimpath --ldflags "$(LDFLAGS)" -o $(PWD)/go-cqhttp.exe 1>/dev/null
